...
> 폴더 / 패키지명 `ai_news_scraping` 는 첫 use case 가 AI 도메인이라 붙은 historical 이름입니다. **도구 자체는 generic** — 키워드/매체 config 만 갈아끼우면 다른 주제도 운영 가능.

매일 아침 영어권 주요 매체의 **특정 주제 기사** (현재 default: AI) 를 자동 수집·요약·한국어로 정리해 메일로 전달하는 재사용 가능 도구. AI 외 도메인 (헬스 / 핀테크 / IT 등) 도 `domains/<이름>/` 폴더에 `keywords.yaml` + `sources.yaml` 만 추가하면 동일 파이프라인으로 운영 가능 (CLAUDE.md §3.1).
