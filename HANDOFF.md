...
> 작성일: 2026-05-23 · 최근 갱신: **2026-05-25 (4회차)** · 인계 시점 commit: `d1dc517` · **363 tests pass**
>
...
> **2026-05-25 (4회차) 갱신 요약**: ① 옵션 A UX 강화 — H5 (검출 키워드 input + row 별 🔍 버튼 + 결과 0 친절 안내) + H6 (전체 active 키워드 합산 모드 + UI 체크박스) + H7 (Brave rate limit 우회: 키워드 사이 1.2초 sleep) + H8 (default 합산 + 페이지 전체 spinner overlay). ② commits `bf29897..d1dc517` (5개 추가). 349 → 363 tests (+14). ③ **운영 권장 흐름 확정** (§12-C 갱신): 호스트 14개 등록 → cron 활성 → 1~2주 운영 → 잡음 매체만 자동 검출로 path 좁히기 (옵션 A 는 첫 등록보다 fine-tuning 단계 도구). per-source cap 3 이 매체 편향 자동 해결. ④ 다음 세션 task **없음** — 운영 시작 단계.

> **2026-05-25 (3회차) 갱신 요약**: ① **옵션 A (자동 path 추천) 완료** — Phase H. 사용자가 admin Sources 폼에 도메인만 입력 → "🔍 자동 path 검출" 버튼 → Brave 1회 호출 → 실제 글 path frequency 분석 → 추천 카드 (count/percentage/sample URL) → 선택 시 도메인 input 자동 채움 → 기존 추가 폼으로 등록. 매체 편향 사고 (2회차) 의 근본 해결. ② commits `5ce0e33` (H1 helper) → `3ecaf89` (H2 라우트) → `79c9a08` (H3 UI). 329 → 349 tests (+20). ③ 다음 세션 task **없음** — 현재 운영 가능 상태.

> **2026-05-25 (2회차) 갱신 요약**: ① 운영 사고 1번 — 메일 결과가 **Anthropic 1개 매체로 92% 쏠리는 편향**. 원인: **사용자 등록 path 가 실제 글 URL 패턴과 불일치** (예: `openai.com/news` 등록인데 실제 글은 `/index/<slug>`). 매체 14개 active 인데 13개 매체 결과 0. ② 해결: cap 권장 공식 강화 (×1.5 → /2 분산 강제), `_looks_like_article_url` 휴리스틱 완화 (`/index` `/blog` 등 일반 글 path 더 통과), search 디버그 로깅 (단계별 reject 카운트). ③ 운영 측 변경 필요 — admin Sources path 수정 또는 호스트만 등록 (§12-D 참조). ④ **다음 세션 작업: 옵션 A — 자동 path 추천** (3회차에 완료, §12-C 참조). ⑤ commits `d053709..0bb7d86` (10개 추가). 320 → 329 tests (+9).
...
| 영역 | 상태 |
|------|------|
| 코드 산출물 | ✅ Phase A~H 완료 — 옵션 A + UX 강화 (2026-05-25 4회차) 까지 (T1~T15 + H1~H8) |
| 테스트 | ✅ **363 passed** (lint/mypy/pytest 모두 exit 0) |
...
| 매체 편향 사고 (5/25) | ✅ **해결 완료** — 옵션 A (자동 path 추천) 로 사용자 시행착오 0. admin 도메인만 입력 → 자동 검출 → 선택 → 등록. §12-C 참조 |
| admin Sources path 운영 | ✅ "🔍 자동 path 검출" 버튼으로 실제 글 path 자동 추천. 다중 path 는 반복 검출/선택/등록 |
...
| ~~🔥 ⭐⭐⭐⭐⭐~~ ✅ **완료** | ~~옵션 A — 자동 path 추천~~ — 2026-05-25 3회차 구현 완료 (H1~H3). admin Sources 폼에 "🔍 자동 path 검출" 버튼 + Brave 1회 호출 + 추천 카드. §12-C 참조. | — |
...
### 12-C. ✅ 옵션 A — 자동 path 추천 — 완료 (2026-05-25 3회차)

**완료 commits**: H1 (`5ce0e33`) → H2 (`3ecaf89`) → H3 (`79c9a08`). 329 → 349 tests (+20).

**최종 형태** (운영 사용법):

1. admin Sources 탭 진입
2. 추가 폼의 **도메인 input 에 도메인만 입력** (예: `openai.com`). path 몰라도 OK.
3. **"🔍 자동 path 검출"** 버튼 클릭
4. admin 백엔드:
   - `POST /admin/sources/discover-paths` (Form: `domain=openai.com`)
   - `search.discover_paths(host, keyword)` 호출 — keyword 는 active 키워드 첫 번째 (없으면 "AI")
   - Brave Search 1회 호출 (`site:openai.com` + keyword, freshness=pm, count=20)
   - 응답 URL 들의 **첫 segment frequency** 집계 (`/index` 13건, `/research` 3건, ...)
   - top 5 `PathSuggestion(prefix, count, percentage, sample_url)` JSON 반환
5. UI 가 결과 카드 표시:
   ```
   🔍 openai.com 의 실제 글 path 패턴
   키워드 "AI" 로 Brave 검색한 결과...

   openai.com/index        13건 (65%)   예시 ↗   [선택]
   openai.com/research     3건  (15%)   예시 ↗   [선택]
   openai.com/blog         1건  (5%)    예시 ↗   [선택]
   ```
6. 사용자가 1개 "선택" 클릭 → 도메인 input 에 `openai.com/index` 자동 채워짐 + 카드 닫힘
7. 사용자가 이름 입력 (예: "OpenAI Index") → "추가" 클릭 → 기존 등록 흐름
8. 다중 path 필요 시 단계 3~7 반복 (`openai.com/research`, `openai.com/blog` 각각 등록)

**핵심 구현 파일**:

- `src/ai_news_scraping/search.py` — `PathSuggestion` dataclass + `discover_paths(host, keyword, *, api_key, num=20, freshness="pm", top_n=5, session=None)`. host normalize / keyword 검증 / Brave 호출 + URL 첫 segment 집계 + count DESC stable sort
- `src/ai_news_scraping/admin.py` — `POST /admin/sources/discover-paths` 라우트 + `create_app(brave_search_api_key, discover_paths_fn)` 추가 (테스트 주입). keyword 빈 → active 첫 번째 → "AI" default. 외부 ValueError → 400, RuntimeError → 503
- `src/ai_news_scraping/cli.py` — `create_app` 호출 시 `settings.brave_search_api_key` 자동 wiring
- `templates/admin.html` — Sources 폼 옆 "🔍 자동 path 검출" 버튼 + 결과 카드 JS (fetch + escape HTML + 선택 시 domain input 자동 채움 + showToast 통합)
- `tests/test_search.py` — discover_paths 10 단위 테스트 (집계 / sort / empty / 다른 host 무시 / 루트 URL 무시 / www 정규화 / 인자 검증 / top_n / stable sort)
- `tests/test_admin.py` — discover-paths 라우트 9 통합 테스트 (정상 / www 정규화 / keyword default / "AI" fallback / 503 api_key 누락 / 400 빈 domain / 400 ValueError / 503 외부 에러 / auth)

**기대 효과 (실측)**:
- 사용자 시행착오 0 — 도메인만 입력
- 정확도 자동 보장 — Brave 실제 색인 path 기반
- 운영 1주 후 매체별 평균 발송 수 통계로 효과 검증 권장 (다음 후보)

**다음 운영 액션** (대표님):
1. `git pull` → 옵션 A 코드 반영
2. admin Sources → 기존 Anthropic 외 13 매체 (path 잘못 등록된 것들) 삭제
3. 도메인만 가지고 자동 검출 + 등록 재진행
4. ▶ 강제발송 → 매체 다양성 검증

---

### 12-C-α. ⭐ 운영 권장 흐름 (4회차 합의)

대표님과 4회차에 합의된 운영 시작 권장 흐름. 결정 마비 회피 + 데이터 기반 fine-tuning.

**핵심 인식**:
- **per-source cap 3** 이 매체 편향 사고를 자동 해결 (호스트 등록이든 path 등록이든 무관). 매체 14 × cap 3 = 매체별 균형 분포 강제.
- path 좁히기는 **잡음 차단 추가 보강** 일 뿐 — 휴리스틱 (`_looks_like_article_url`) 이 카테고리/제품/이벤트/careers 등 자동 차단 (70~80% 처리).
- 옵션 A 자동 검출은 **첫 등록 도구가 아니라 운영 중 fine-tuning 도구**.

**권장 운영 흐름** (대표님 운영 시작 시):

1. **호스트 14개 등록** (admin Sources, 15분)
   - 도메인만 (예: `openai.com`, `anthropic.com`, `techcrunch.com`)
   - path 절대 좁히지 말 것 — 휴리스틱 + cap 3 에 위임
2. **per-source cap 3 확인** (admin Settings → "매체당 최대 결과 수" = 3)
3. **GitHub Actions secrets 8개 등록 + 수동 trigger 1회 검증** ([`cron-setup-guide.html`](./cron-setup-guide.html))
4. **cron 활성 + 1~2주 운영**
5. **메일 본문 살펴봄** — 잡음 글 (마케팅/이벤트/HR) 자주 통과되는 매체 식별
6. **잡음 매체만 자동 검출 → path 좁히기** (옵션 A 활용)
   - admin Sources → 해당 row 의 🔍 path 검출 버튼
   - 합산 모드 (default) → 추천 받음 → 좁힌 row 새로 추가 → 기존 호스트 row 삭제
7. **매체별 평균 발송 수 통계로 검증** (후속 후보 §12 표 참조)

**핵심 의사결정 근거** (대표님 솔직 토론 결론):
- A (호스트만) / B (path 좁힘) / C (split 실험) 어느 쪽도 결정적 우위 X
- 운영 시작이 결정 마비보다 중요
- 호스트만 시작 = 시행착오 0 + 옵션 A 가 fine-tuning 비용 0 으로 만들어 둠

**사전 옵션 (선호 시)**:
- 매체별로 자동 검출 결과만 미리 한 번씩 확인해두기 (UI 에서 클릭만, 등록 X). 1~2주 후 잡음 보이면 미리 본 검출 결과 기억으로 즉시 좁히기.

---

### 12-D. 매체 편향 사고 원본 기록 (2026-05-25 2회차, 옵션 A 도입 배경)

**왜 필요했는가** (2026-05-25 2회차 운영 사고):
...
**옵션 A 완료 결과**: §12-C 참조. PRD 사이클 없이 ralph 매 iteration 흐름으로 H1~H3 task 분해 진행 — 1 commit per task. 새 PRD 폴더 (`docs/features/2026-05-26-source-auto-path-suggest/`) 는 생성하지 않음 (HANDOFF.md 의 §12-C 본 섹션이 충분히 sufficient).
...
| H1: discover_paths 첫 segment 만 집계 (depth=1) | 깊은 prefix (예: `/news/2026/05/x`) 까지 집계하면 unique paths 갈라져 frequency 의미 X. 첫 segment 가 매체 구분 단위 (`/index` vs `/research` vs `/blog`) 와 자연스럽게 일치. depth=1 도 60% 이상 정확도 (대부분 매체에서). 사용자가 더 깊은 path 원하면 자동 검출 결과 선택 후 input 에서 수동 추가 가능. |
| H1: freshness default pm (past month) | discover 의 목적은 **path 패턴 발견** — 검색 결과 풍부 우선. 실제 검색 (search 함수) 은 pd/pw 짧게 잡지만, discover 만 pm 으로 한 번 호출. 호출 비용 = +1 query / 매체 등록 시점 (운영 ~월 14회 정도). |
| H2: keyword 자동 default 흐름 (active 첫 번째 → "AI") | 사용자가 admin 등록 시 keyword 모름. active 키워드 첫 번째 사용 = "어차피 같은 키워드로 운영 중인 매체에 추가하는 것" 가정. 빈 keyword_store 의 edge case 는 "AI" hardcode default. |
| H2: discover_paths_fn 의존성 주입 (테스트용) | 외부 Brave HTTP 호출 mock 하기 위해 함수 자체 주입. 기본은 `search.discover_paths` 직접 사용 — 운영에서 인자 추가 X. 같은 패턴이 admin.py 의 `run_pipeline` callback 에서도 사용 (테스트 친화). |
| H3: 단순 single-pick UI (vs 다중 선택 일괄 등록) | 첫 구현은 가장 적은 코드 + 기존 폼 재활용. 다중 등록 사용자가 반복 (검출 → 선택 → 추가 → 검출 ...) 으로 처리 — 매체 추가 빈도 (월 ~2회) 고려 시 충분. 운영해보고 불편하면 다중 일괄 등록 후속 task. |
| H3: domain input 자동 채움 (vs 다른 hidden field) | 기존 폼 그대로 사용 — admin.html 의 form action="/sources" 라우트 변경 없음. 사용자 검증 (예: input 값 직접 수정) 가능. zero 리팩터링. |
| H4: PRD 사이클 (brainstorming → tech-design → plan → execute) 생략, ralph 직접 진행 | HANDOFF.md §12-C 가 이미 작동 흐름 / 구현 task / 기대 효과 모두 명시. 옵션 A scope 가 task 3개 (H1~H3) 로 작아서 PRD 4단계 오버킬. 매 iteration 1 task → commit 흐름이 더 효율. |
| H5: UI 에 keyword input + 행별 🔍 버튼 (vs 자동 keyword fallback) | 사용자가 anthropic.com 합산 검출 시 결과 0. 원인: active 첫 키워드 색인 X. fallback (둘째 → 셋째 → "AI") 은 호출 N배 부담. UI input 으로 사용자 명시 선택권 + 행별 버튼으로 기존 row fine-tuning. |
| H6: 합산 모드 search.py 에 helper 분리 (vs admin 에서 직접 N 호출) | `_aggregate_first_segments` / `_brave_search_items` helper 분리 → 단일/합산 코드 path 공유 + 테스트 단순. admin 은 thin wrapper 만. |
| H6: 합산 default off (체크박스 opt-in) → H8 에서 default on 변경 | H6 first cut 은 보수적 — 호출 비용 명시 인지. 실제 운영해보니 합산이 정확도 훨씬 우월. H8 에서 default on 으로 전환 (사용자 의도와 일치). |
| H7: discover_paths_multi 에 sleep 1.2초 (vs retry on 429) | pipeline.py 패턴 그대로 동기 sleep — Brave Free 1 query/sec 제한 회피. retry 는 429 후 더 큰 지연 필요 + 코드 복잡. 단순 sleep 이 회귀 가능성 ↓. |
| H7: delay_seconds 인자 expose | 테스트는 0 명시 (sleep skip) + 운영은 1.2 default. 같은 pattern (pipeline 의 search_delay_seconds) 과 일관. |
| H8 (fast-tasks): default 합산 모드로 전환 | 사용자 의도 명시 + 14 매체 × 색인 분산 케이스에서 단일 키워드 검출이 약함 (단일 키워드 색인 0 매체가 다수). 합산이 default 일 때 운영 자연스러움. |
| H8: 페이지 전체 spinner overlay (vs discoverResult 안 작은 spinner) | 합산 모드 latency 15초+ — 사용자가 다른 버튼 클릭하면 race condition. 전체 차단 backdrop 으로 abort 막고 "기다리는 중" 명시. |
| 4회차: 운영 권장 호스트만 시작 (vs path 첫 등록부터 좁히기) | 솔직 토론 결과 — 데이터 없이 path 좁히기는 추론. per-source cap 3 이 매체 편향 자동 해결 + 휴리스틱이 잡음 70~80% 처리. 호스트 시작 + 1~2주 데이터로 fine-tune 이 합리적 시퀀스. 옵션 A 가 fine-tuning 비용 0. |
...
- **개발 도구**: Claude Code + ralph-loop 플러그인 + js-super skills (PRD → tech-design → plan → execute-plan 자동화 흐름 + ralph 직접 task 분해 흐름 + fast-tasks 묶음 처리 혼용)
- **개발 기간**: 2026-05-23 (vision-intake → Phase A~G + fast-tasks) + 2026-05-24 (search-path-prefix) + 2026-05-25 (admin-send-schedule, brand, cron tuning, per-source cap, 권장값 카드, 휴리스틱 완화 — 2회차) + 2026-05-25 (옵션 A 자동 path 추천 H1~H3 — 3회차) + 2026-05-25 (옵션 A UX 강화 H5~H8 + 운영 권장 흐름 — 4회차)
- **총 commit 수**: 78+ (linear history, main 브랜치)
- **다음 세션 첫 task**: **없음** (운영 시작 단계). §12-C-α 의 권장 흐름 따라 운영 → 1~2주 후 데이터 보고 fine-tune 결정. 후속 후보 (매체별 통계 / runs.trigger 종류 / drill-down) 는 §12 표 참조.

### 4회차 인계 핵심 요약 (대표님이 다음 세션 시작 시 5분 안에 파악)

1. **상태**: 코드 완성. 운영 시작 단계. 363 tests pass.
2. **운영 첫 task**: §12-C-α 권장 흐름 — 호스트 14개 등록 → cron 활성 → 1~2주 운영.
3. **자동 검출 사용법**: admin Sources 폼 (추가 시) + 각 row 의 🔍 버튼 (fine-tuning 시). default 합산 모드 + 페이지 전체 overlay 로 사용자 인지 보장.
4. **운영 사고 자가 진단**: 메일 본문이 한 매체로 쏠리면 → per-source cap 3 확인 (admin Settings). 잡음 글 (마케팅/HR) 자주 통과되면 → 그 매체만 자동 검출 → path 좁히기.
5. **건드릴 필요 없는 것**: 휴리스틱 (`_looks_like_article_url`), cron 표현식 (`*/5 23,0 * * *`), 마이그레이션 0001~0005. 모두 운영 검증 끝났거나 변경 시 회귀 risk 큼.
